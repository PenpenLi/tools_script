...
-(void)draw {
    ...
    // Arreglo, para que no se vean los rayos negros
...
-(void)strike
{
    ...
    if (_autoRemoveFromParent) {
        action = [CCSequence actionOne:action two:[CCCallFuncND actionWithTarget:self selector:@selector(removeFromParentAndCleanup:) data:(void*)TRUE]];
    } else {
