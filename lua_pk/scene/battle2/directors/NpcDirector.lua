...
function NpcDirector:checkWalk()
    ...
    if standInfo.target == nil then return true end
    ...
    self:setTimeout(math.floor(dt*30), function()
        if self.attackTarget then
            ...
            if self.isKuangBao then
                ...
                self:onAddHp(self.key, math.max(attackHp*0.05, 10))
            end
        end
    end)
...
function NpcDirector:getSmallSkillParam()
    ...
    param.hp = self.npcBook.smallSkillAttackVal
    ...
    for i=1, #self.bigSkillTargets do
        ...
        self:setTimeout(math.floor(dt*30), function ()
            ...
            if self.npcBook.name == "XianXueNvWangLan" then
                local addHp = math.max(self:getBigSkillHp()*0.05, 10)
...
function NpcDirector:getBigSkillParam()
    local param = {}
    param.fromKey = self.key
    param.hp = self.npcBook.bigSkillAttackVal
...
function NpcDirector:onAttack(fromKey,hp)
    ...
    if effect.hurt ~= 0 then
        hurt = math.max(effect.hurt - self:getArmorVal(),10+math.floor(self.npcBook.npcLevel/100))
...
function NpcDirector:onSmallSkill(fromKey, smallSkillParam)
    ...
    elseif fromDirector.npcBook.name == "YiLiDan" then
        self:startJianFang()
        self:setTimeout(5, function() self:stopJianFang() end)
...
function NpcDirector:onBigSkill(fromKey, bigSkillParam)
    ...
    elseif fromDirector.npcBook.name == "WaSiQi" then
        for i=1, 4 do
            ...
            self:setTimeout(time, function()
                self:onPersistentSkill(fromKey, self:getBigOnGoingHp())
            end)
        end
...
function NpcDirector:onShengGuang(seq)
    self.isXuRuo = false
    self.isZhongDu = false
    actorAgent:actCMD({t="shengguang_s", key=self.key, seq=seq})
    self:setTimeout(30*5, function() 
        actorAgent:actCMD({t="shengguang_o", key=self.key})
    end)
end

function NpcDirector:onTuTeng(seq,pos)
    self.isTuTeng = true
    ...
    if npcBook.row and npcBook.col then
        ...
        if self:isTower() then
            if not (npcBook.row and npcBook.col) then
                ...
                self.row = cell.row + 5
                self.col = cell.col
                if self.npcBook.npcId==20000 then
                    self.row = self.row-4
                end
...
function NpcDirector:startJianFang()
    print("[NpcDirector]startJianFang")
    self.jianFangVal = 100
...
function NpcDirector:rollEffectType(fromKey, hp)
    ...
    elseif randomNum>self.npcBook.cri+self.npcBook.block and
        randomNum<=self.npcBook.cri+self.npcBook.block+from.npcBook.dex-self.npcBook.agl then
        return {eft=2, hurt=hp*0}
    else
