...
function HomeUILayer:initM()
    ...
    if Roler:get("avatarId") == 1 then
        self.m_headImg:loadTexture("touxiang.png")
	else
        self.m_headImg:loadTexture("touxiang1.png")
	end
	...
	--m_juexingBtn: Button
	self.m_juexingBtn = ccui.Helper:seekWidgetByName(self.rootNode, "m_juexingBtn")
	self.m_juexingBtn:addTouchEventListener(touchEventHandler)
    ...
    --self.m_GemBtn:loadTextures("xinghun.png","xinghun02.png","xinghun02.png")
    ...
    --buttons: Layout
    self.buttonsLayout = ccui.Helper:seekWidgetByName(self.rootNode, "buttons")
...
function HomeUILayer:updateTipPic(reslist)
...
function HomeUILayer:refreshLabelUI()
    ...
    local m_alltime = m_onetime*tili_need
    
    local function UpdateSome(dt)
        if tonumber(Roler:get("energyMax"))-tonumber(Roler:get("energy")) ~= tili_need then
            tili_need = tonumber(Roler:get("energyMax"))-tonumber(Roler:get("energy"))
            tili_need = math.max(tili_need,0)
            m_onetime = tonumber(Roler:get("recoverEnergyInterval"))*60
            m_alltime = m_onetime*tili_need
        end
...
function HomeUILayer:openBuyTip(type)
    ...
    if type == "1" then
        ...
        Panel_tip1:setVisible(true)
    elseif type == "2" then
        Panel_tip1:setVisible(false)
        Panel_tip2:setVisible(true)
    end
end
                ...
                elseif sender == buy1btn and event == cc.EventCode.ENDED then
                    ...
                    if buynum <= self.m_buytilinum then
                        Http:req("BuyCoins",{type = 1},function (res)
                            ...
                            elseif res.data["result"] == 5 then
                                PopDialog:getInstance():popText("体力已满")
                            else
                                self:openBuyTip("2")
                            end
                        end)                    
...
function HomeUILayer:showChatUI(data)
    ...
    if m_ChatUI == nil then
        ...
        local m_rptPanel = ccui.Helper:seekWidgetByName(m_ChatUI, "m_rptPanel")
        m_rptPanel:setVisible(false)
        
        local surebtn = ccui.Helper:seekWidgetByName(m_ChatUI, "surebtn")
        local canclebtn = ccui.Helper:seekWidgetByName(m_ChatUI, "canclebtn")
        ...
        local function ChatBtnCallBack(sender, event)
            ...
            elseif sender == surebtn and event == cc.EventCode.ENDED then
                ...
                Http: req("GetBattleReport",
                    {rptId = self.chat_rptid,type = 1},
                    function(res) 
                        if res.err then cclog("err!!!") return end
                        local jjc = require "scene.sport.SportUILayer"
                        if jjc:checkResult(res.data["result"]) then
            ...
            elseif sender == sendbtn and event == cc.EventCode.ENDED then
                ...
                Http:req("Chat",{type = self.chattype,message = chatstring},function(res)
                    ...
                    elseif res.data["result"] == 3 then
                        PopDialog:getInstance():popText("等级不足，无法聊天")
        ...
        surebtn:addTouchEventListener(ChatBtnCallBack)
        canclebtn:addTouchEventListener(ChatBtnCallBack)
...
function HomeUILayer:updateChatUI(data)
    ...
    if self.chattype == 0 then
        ...
        for i=table.getn(m_chatSJ), 1,-1 do
            local stringtabel = string.split(m_chatSJ[i]["message"],"|")
            
            local chatItem = ccui.Layout:create()

            local img = ccui.ImageView:create()
            img:loadTexture("chat/quan.png")
            chatItem:addChild(img,1)
            ...
            chatItem:addChild(namelabel,1)
            namelabel:setColor(cc.c3b(69,42,19))

            local chatlabel = ccui.Text:create()
            chatlabel:setFontSize(20)
            chatlabel:setAnchorPoint(0,1)
            chatlabel:setColor(cc.c3b(69,42,19))
            
            if stringtabel[1] == "/rpt" and table.getn(stringtabel) == 4 then
                chatlabel:setString(stringtabel[2])
                chatlabel:setColor(cc.c3b(255,234,79))
                chatItem:addChild(chatlabel,1)
                
                local chatlabel2 = ccui.Text:create()
                chatlabel2:setFontSize(20)
                chatlabel2:setAnchorPoint(0,1)
                chatlabel2:setString(stringtabel[4])
                chatItem:addChild(chatlabel2,1)
                
                chatlabel:setTouchEnabled(true)
                
                local function rptCallBack(sender, event)
                	if sender == chatlabel and event == cc.EventCode.ENDED then
                        self.chat_rptid = sender:getTag()
                        
                        local m_rptPanel = ccui.Helper:seekWidgetByName(m_ChatUI, "m_rptPanel")
                        m_rptPanel:setVisible(true)
                	end
                end
                
                chatlabel:addTouchEventListener(rptCallBack)
                chatlabel:setTag(tonumber(stringtabel[3]))
                
                local labelheight = (math.floor(chatlabel2:getStringLength()/23)+1)*33
                chatlabel2:setContentSize(535,labelheight)
                chatlabel2:ignoreContentAdaptWithSize(false)

                local Panelheight = 25+labelheight+xian:getContentSize().height+namelabel:getContentSize().height+16

                xian:setPosition(4,0)
                img:setPosition(25,Panelheight/2)
                namelabel:setPosition(48,Panelheight-8)
                chatlabel:setPosition(48,Panelheight-40)
                chatlabel2:setPosition(48,Panelheight-65)
                ...
                local labelheight = (math.floor(chatlabel:getStringLength()/23)+1)*33
...
function HomeUILayer:handleTouchEvent(sender, event)
	...
	elseif sender == self.m_startServerActBtn and event == cc.EventCode.ENDED then
        ...
        Http: req("GetOpenActRequest",
            {actId = 1},
            function(dat) 
                if dat.err then cclog("err!!!") return end
                local scene = require("scene.openAct.OpenActScene"):create(dat)
	...
	elseif sender == self.m_juexingBtn and event == cc.EventCode.ENDED then
		cclog("[HomeUILayer]--m_juexingBtn touched--")
        local scene = require("scene.juexing.JueXingScene"):create()
        cc.Director:getInstance():replaceScene(scene)
	...
	elseif sender == self.m_dailyBtn and event == cc.EventCode.ENDED then
        ...
        Http: req("GetMainDt",
            ...
            function(res) 
                if res.err then cclog("err!!!") return end 
                local scene = require("scene.dailytask.DailyTaskScene"):create(res)
                cc.Director:getInstance():pushScene(scene)
            end)
    ...
    elseif sender == self.m_GemBtn and event == cc.EventCode.ENDED then
        ...
        if Roler:get("level") < tonumber(OpenConditions["3"]["need_level"]) then
            PopDialog:getInstance():popText("宝石功能"..OpenConditions["3"]["need_level"].."级开放")
            return
        end
...
function HomeUILayer:randomName()
...
function HomeUILayer:showSettingLayer()
        ...
        local function SettingBtnCallBack(sender, event)
            ...
            elseif sender == surebtn3 and event == cc.EventCode.ENDED then      
                ...
                Http:req("ChangeName",{name = username:getStringValue()},function(res)
                    ...
                    elseif res.data["result"] == 1 then
                        PopDialog:getInstance():popText("名字已存在!")
                    elseif res.data["result"] == 2 then
                        PopDialog:getInstance():popText("名字不合法!")
                    elseif res.data["result"] == 3 then
                        PopDialog:getInstance():popText("钻石不足!")
                    end
                end)
            ...
            elseif sender == suijibtn and event == cc.EventCode.ENDED then
                    ...
                    local username = ccui.Helper:seekWidgetByName(Panel_name, "TextField")
                    username:setText(self:randomName())
    ...
    if Roler:get("avatarId") == 1 then
        touxiang:loadTexture("touxiang.png")
    else
        touxiang:loadTexture("touxiang1.png")
    end
