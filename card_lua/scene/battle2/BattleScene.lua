...
function BattleScene:createEmulate()
    ...
    local emulateBattleParam = cjson.decode(emulateBattleJson)
    emulateBattleParam.isEmulate = true
    --emulateBattleParam.isAutoFight = true
...
function BattleScene:initM(param)
    ...
    local battleConfig = {
        ...
        towerNpcData = {
            {
                {npcId=20010, npcLevel=1, normalAttackVal=200, smallSkillLevel=1, smallSkillAttackVal=0, bigSkillLevel=1, bigSkillAttackVal=0, armorVal=20, maxHp=200, currHp=200,treasuresNum=0, cri=10, block=0,dex=10,agl=0,soulId=9000,attackInterval=2000,smallSkillInterval=9999,bigSkillInterval=9999,zhili=100},
                {npcId=20011, npcLevel=1, normalAttackVal=200, smallSkillLevel=1, smallSkillAttackVal=400, bigSkillLevel=1, bigSkillAttackVal=600, armorVal=20, maxHp=2000, currHp=2000,treasuresNum=0, cri=10, block=10,dex=10,agl=10,soulId=9000,attackInterval=2000,smallSkillInterval=2,bigSkillInterval=4,zhili=100},
                {npcId=20005, npcLevel=1, normalAttackVal=200, smallSkillLevel=1, smallSkillAttackVal=400, bigSkillLevel=1, bigSkillAttackVal=600, armorVal=20, maxHp=200, currHp=200,treasuresNum=0, cri=10, block=10,dex=10,agl=10,soulId=9000,attackInterval=2000,smallSkillInterval=2,bigSkillInterval=4,zhili=100},
                {npcId=20004, npcLevel=1, normalAttackVal=200, smallSkillLevel=1, smallSkillAttackVal=400, bigSkillLevel=1, bigSkillAttackVal=600, armorVal=20, maxHp=200, currHp=200,treasuresNum=0, cri=10, block=10,dex=10,agl=10,soulId=9000,attackInterval=2000,smallSkillInterval=2,bigSkillInterval=4,zhili=100},
                {npcId=20000, npcLevel=1, normalAttackVal=200, smallSkillLevel=1, smallSkillAttackVal=400, bigSkillLevel=1, bigSkillAttackVal=600, armorVal=20, maxHp=200, currHp=200,treasuresNum=0, cri=10, block=10,dex=10,agl=10,soulId=9000,attackInterval=2000,smallSkillInterval=2,bigSkillInterval=4,zhili=100},
                {npcId=20017, npcLevel=1, normalAttackVal=200, smallSkillLevel=1, smallSkillAttackVal=400, bigSkillLevel=1, bigSkillAttackVal=600, armorVal=20, maxHp=200, currHp=200,treasuresNum=0, cri=10, block=10,dex=10,agl=10,soulId=9001,attackInterval=4000,smallSkillInterval=20000,bigSkillInterval=20000,zhili=100},
                {npcId=20014, npcLevel=1, normalAttackVal=200, smallSkillLevel=1, smallSkillAttackVal=400, bigSkillLevel=1, bigSkillAttackVal=600, armorVal=20, maxHp=200, currHp=200,treasuresNum=0, cri=10, block=10,dex=10,agl=10,soulId=9000,attackInterval=4000,smallSkillInterval=20000,bigSkillInterval=20000,zhili=100},
                {npcId=20015, npcLevel=1, normalAttackVal=200, smallSkillLevel=1, smallSkillAttackVal=400, bigSkillLevel=1, bigSkillAttackVal=600, armorVal=20, maxHp=200, currHp=200,treasuresNum=0, cri=10, block=10,dex=10,agl=10,soulId=9000,attackInterval=4000,smallSkillInterval=20000,bigSkillInterval=20000,zhili=100},
                {npcId=20016, npcLevel=1, normalAttackVal=200, smallSkillLevel=1, smallSkillAttackVal=400, bigSkillLevel=1, bigSkillAttackVal=600, armorVal=20, maxHp=200, currHp=200,treasuresNum=0, cri=10, block=10,dex=10,agl=10,soulId=9000,attackInterval=4000,smallSkillInterval=20000,bigSkillInterval=20000,zhili=100},
    ...
    local isAutoFight = param.isAutoFight or false
...
function BattleScene:loadAsync(my, enemy1, enemy2, enemy3)
    ...
    local define_npc_res = require "config.define_npc_res"
    local resList = {}
    
    local function findWeatherInTable(srctable,value)
        for i=1,#srctable do
            if  srctable[i] == value then
                return true
            end
        end
        return false
    ...
    local function getHeroSkilResList(id)
        local soulId = tostring(id)
        local list = {}
        local soulresItem = define_npc_res[soulId]
        if soulresItem then
            if  soulId == 30 then
                print(30)
            end
            local normalSkillList =  string.split(soulresItem["normalAttackRes"],",")
            local smallSkillResList =  string.split(soulresItem["smallSkillRes"],",")
            local bigSkillResList =  string.split(soulresItem["bigSkillResRes"],",")
            for i=1,#normalSkillList do
                if  normalSkillList[i] ~="" and normalSkillList[i] ~= "nil" then
                    table.insert(list,normalSkillList[i])
                end
            end
            for i=1,#smallSkillResList do
                if  smallSkillResList[i] ~="" and smallSkillResList[i] ~= "nil" then
                    table.insert(list,smallSkillResList[i])
                end
            end
            for i=1,#bigSkillResList do
                if  bigSkillResList[i] ~="" and bigSkillResList[i] ~= "nil" then
                    table.insert(list,bigSkillResList[i])
                end
            end
        end
        return list
    ...
    local function addResList(memberList)
        for i=1, #memberList do
            ...
            if  findWeatherInTable(resList,path) == false then
                table.insert(resList, path)
            end
            
            local heroResList = getHeroSkilResList(soulKey)
            for i=1,#heroResList do
                if  findWeatherInTable(resList,heroResList[i]) == false then
                    table.insert(resList, heroResList[i])
                 end
            end
        end
    end
    ...
    Helper.setTimeout(self, 5, function()
        for i=1, #resList do
            print(resList[i].."  num:"..i.."/"..#resList)
            local tmpSplit = string.split(resList[i],".")
            local tmpType = tmpSplit[2]
            if  tmpType == "csb" then
                ccs.ArmatureDataManager:getInstance():addArmatureFileInfoAsync(resList[i], loadOk)
            else
                cc.Director:getInstance():getTextureCache():addImageAsync(resList[i],loadOk)
            end
        end
    end)
end
...
function BattleScene:transFormData(param)
    ...
    local function parseData(roundsData)
        ...
        for i=1,table.getn(roundsData) do
            ...
            tempTable.big_skill_range_x = 12
            tempTable.small_skill_range_x = 12
            if smallSkill~=nil and bigSkill~=nil then
                ...
                tempTable.big_skill_range_x = bigSkill["skill_range_x"]
                tempTable.big_skill_range_y = bigSkill["skill_range_y"]
                tempTable.big_add_def_factor = bigSkill["add_def_factor"]
                tempTable.big_add_baoji_factor = bigSkill["add_baoji_factor"]
                tempTable.big_add_gedang_factor = bigSkill["add_gedang_factor"]
                tempTable.big_add_hit_factor = bigSkill["add_hit_factor"]
                tempTable.big_add_miss_factor = bigSkill["add_miss_factor"]
                
                tempTable.small_skill_range_x = smallSkill["skill_range_x"]
                ...
                tempTable.small_skill_range_y = smallSkill["skill_range_y"]
                tempTable.small_add_def_factor = smallSkill["add_def_factor"]
                tempTable.small_add_baoji_factor = smallSkill["add_baoji_factor"]
                tempTable.small_add_gedang_factor = smallSkill["add_gedang_factor"]
                tempTable.small_add_hit_factor = smallSkill["add_hit_factor"]
                tempTable.small_add_miss_factor = smallSkill["add_miss_factor"]
            else
