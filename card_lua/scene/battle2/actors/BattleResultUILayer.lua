...
function BattleResultUILayer:initUI(data)
    ...
    local function zhujiaoLevleUpFunc()
        if  isZhuJiaoLevelUp then            
            ...
            local seq = cc.Sequence:create(cc.Hide:create(),cc.DelayTime:create(1.0),cc.Show:create(),scaleTo,cc.ScaleTo:create(0.25,1.0,1.0))
            self.m_TeamLevelUpPanel:runAction(seq)
            ...
            local guangBG = ccui.Helper:seekWidgetByName(self.m_TeamLevelUpPanel,"m_guangImage")
            guangBG:runAction(cc.RepeatForever:create(cc.RotateBy:create(2.0,180)))
        end
    end
    ...
    if  self.battleType == 5 then
        ...
        local m_guangImage = ccui.Helper:seekWidgetByName(self.m_BattlleFinishImage,"m_guangImage")
        --[[
        ...
        m_guangImage:runAction(cc.RepeatForever:create(cc.RotateBy:create(2.0,180)))
        
        local armatureFile1 = "image/battle/resultlayer/battleWin_lizi/battleWin_lizi.csb"
        ccs.ArmatureDataManager:getInstance():addArmatureFileInfo(armatureFile1)
        local lizi = ccs.Armature:create("battleWin_lizi")
        lizi:getAnimation():play("Animation1")
        lizi:setPosition(m_guangImage:getPosition())
        self.m_BattlleFinishImage:addChild(lizi)
        ...
        self:runAction(cc.Sequence:create(cc.DelayTime:create(1.2),cc.CallFunc:create(finishFunc),cc.CallFunc:create(zhujiaoLevleUpFunc)))
        ...
        if  data["win"] == 0 then 
            ...
            local TagFailImage = ccui.Helper:seekWidgetByName(self.m_BattleFailImage,"TagFailImage")
            ...
            if data["leftSeconds"] then
                if  data["leftSeconds"] <= 0 then
                    TagFailImage:loadTexture("image/battle/resultlayer/chaoshizi.png")
                end
            end
            ...
            self:runAction(cc.Sequence:create(cc.DelayTime:create(1.2),cc.CallFunc:create(falseFunc),cc.CallFunc:create(zhujiaoLevleUpFunc)))
            ...
            local m_guangImage = ccui.Helper:seekWidgetByName(self.m_BattleWinImage,"m_guangImage")
            ...
            lizi:getAnimation():play("Animation1")
            lizi:setPosition(m_guangImage:getPosition())
            self.m_BattleWinImage:addChild(lizi,3)
            ccs.ArmatureDataManager:getInstance():removeArmatureFileInfo(armatureFile1)
