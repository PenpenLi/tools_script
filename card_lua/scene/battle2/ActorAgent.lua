...
function ActorAgent:actCMD(cmd)
    ...
    if cmd.t == "jx_dark_s" then
        self.bg:pause()
        return
    end
    
    if cmd.t == "jx_dark_o" then
        self.bg:resume()
        return
    end
    ...
    if cmd.t == "fire_no" then
        --print("~~~~~~~~~~~~~~~~~~~~fire_no~~~~~~~~~~~~~~~~~~")
        --self.ui:disableBigSkill(cmd.sid)
        self.ui:fire_no(cmd.sid)
        return
    end
    
    if cmd.t == "fire_yes" then
        --print("~~~~~~~~~~~~~~~~~~~~fire_yes~~~~~~~~~~~~~~~~~~")
        --self.ui:enableBigSkill(cmd.sid)
        self.ui:fire_yes(cmd.sid)
        return
    end
 
    if cmd.t == "suspend_s" then
        self.suspendSeq = cmd.seq
        self.suspendNum = self.suspendNum or 0
        self.suspendNum = self.suspendNum + 1
        self.bg:pause()
        --self.ui:disableBigSkill(cmd.sid)
    end
...
function ActorAgent:onBattleOver(param)
    ...
    --remove the role who not hero
    local function removeNotSoul(attackerList)
        local tmpSoulList ={}
        for i=1,#attackerList do
            if  attackerList[i]["soulId"]<=150 and attackerList[i]["soulId"]>=0 then
                local tmpList = {}
                tmpList.pos = attackerList[i]["pos"]
                tmpList.curHp = attackerList[i]["curHp"]
                table.insert(tmpSoulList,tmpList)
            end
        end
        return tmpSoulList
    end
    ...
    Http: req("NewBattleFinish",
        {endId = math.floor((self.fId-34)/31) ,leftAtk = removeNotSoul(param.leftAtks),leftDef = param.leftDefs,leftSeconds = param.leftTime},
        function(res)
            if res.err then cclog("err!!!") return end
            --local winnner = nil
            print("[ActorAgent] Finish NewBattleFinish")
            res.data.leftSeconds = param.leftTime
