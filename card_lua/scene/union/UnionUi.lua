...
function UnionUi:initM()
    ...
    self.freshpanel = ccui.Helper:seekWidgetByName(self.rootNode,"freshpanel")
    ...
    self.m_percentSlider = ccui.Helper:seekWidgetByName(self.rootNode, "Slider_11")
    self.m_percentSlider:setVisible(false)
    ...
    self.ItemtouxiangPinzhi = ccui.Helper:seekWidgetByName(self.rootNode, "Image_30")
    ...
    self.fontpanel = ccui.Helper:seekWidgetByName(self.rootNode, "Panel_7")
    self.cangkupanel = ccui.Helper:seekWidgetByName(self.rootNode, "cangkupanel")
    ...
    --公会规则相关
    self.m_gonghuiRule = ccui.Helper:seekWidgetByName(self.rootNode, "m_gonghuiRule")
    
    self.m_exitUnionRule = ccui.Helper:seekWidgetByName(self.rootNode, "m_exitUnionRule")
    self.m_exitUnionRule:addTouchEventListener(touchEventHandler)
    
    self.m_guizePanel = ccui.Helper:seekWidgetByName(self.rootNode, "m_guizePanel")
    self.m_guizePanel:addTouchEventListener(touchEventHandler)
    
    --公会大餐相关
    self.coleNum = ccui.Helper:seekWidgetByName(self.rootNode, "Label_103")
    self.hamburgerNum = ccui.Helper:seekWidgetByName(self.rootNode, "Label_112")
    self.chickenNum = ccui.Helper:seekWidgetByName(self.rootNode, "Label_114")
    
    self.AttackNumber = ccui.Helper:seekWidgetByName(self.rootNode, "Label_362")
    
    self.showCangku_2 = ccui.Helper:seekWidgetByName(self.rootNode, "Button_363")
    self.showCangku_2:addTouchEventListener(touchEventHandler)
    
    self.BtnScrollView = ccui.Helper:seekWidgetByName(self.rootNode, "BtnScrollView")
...
function UnionUi:handleTouchEvent(sender, event)
    ...
    elseif sender == self.m_BagDetailListView and event == cc.EventCode.ENDED then
        cclog("物品申请详情")
        --SetSlider(self.m_BagDetailListView,self.m_percentSlider)
    ...
    elseif sender == self.m_DACAN and event == cc.EventCode.ENDED then
        ...
        self.freshpanel:setVisible(false)
        
        self.coleNum:setString(dinnerConfig["1"]["cost"])
        self.hamburgerNum:setString(dinnerConfig["2"]["cost"])
        self.chickenNum:setString(dinnerConfig["3"]["cost"])
    ...
    elseif sender == self.m_UnionShopFreshBtn and event == cc.EventCode.ENDED then
        ...
        self.freshpanel:setVisible(true)
        self.m_CoverPanel:setVisible(true)
    ...
    elseif sender == self.m_confirm5 and event == cc.EventCode.ENDED then
        Http:req("RefreshGuildShopRequest",nil,function(res)
            ...
            if res.data.result == 18 then
                PopDialog:getInstance():popText("刷新失败！今日刷新次数已满",cc.c3b(0,255,0))
            elseif res.data.result == 0 then 
                PopDialog:getInstance():popText("刷新成功！",cc.c3b(0,255,0)) 
                --FreshNum = FreshNum + 1
                self:UnionShopFace()
            elseif res.data.result == 16 then 
                PopDialog:getInstance():popText("刷新失败！您的贡献币不足",cc.c3b(0,255,0))
            end
        end)
        self.m_Sure2Fresh:setVisible(false)
        self.freshpanel:setVisible(false)
    ...
    elseif sender == self.showCangku_2 and event == cc.EventCode.ENDED then
        self.m_Index = 1
    ...
    elseif sender == self.m_CloseCangkuBtn and event == cc.EventCode.ENDED then 
        ...
        if self.m_Index == 0 then
            ...
            self.cangkupanel:setVisible(false)
            self.m_gonghuiRule:setVisible(false)
            self.m_UnionNoticeTitle:setVisible(true)
            self.m_UnionNoticeDetails:setVisible(true)
            self.m_MyUnionInfo:setVisible(true)
            self.m_MyUnionActivity:setVisible(true)
    ...
    elseif sender == self.m_RuleBtn and event == cc.EventCode.ENDED then 
        cclog("仓库规则说明")
        self.m_guizePanel:setVisible(true)
        self.m_gonghuiRule:setVisible(true)
        
    elseif sender == self.m_exitUnionRule and event == cc.EventCode.ENDED then
        self.m_gonghuiRule:setVisible(false)
        self.m_guizePanel:setVisible(false)
        
    elseif sender == self.m_CangkuListView and event == cc.EventCode.ENDED then
        cclog("仓库物品滚动层")
        SetSlider(self.m_CangkuListView,self.m_percentSlider)      
    
    elseif sender == self.m_Cancel10 and event == cc.EventCode.ENDED then
        cclog("否") 
        self.m_sure2Cangku:setVisible(false)
        self.cangkupanel:setVisible(false)
        self.fontpanel:removeChild(self.richtextnot)
    ...
    elseif sender == self.m_UnionBoss and event == cc.EventCode.ENDED then
        ...
        self.m_UnionBg:setVisible(false)
    ...
    elseif sender == self.m_CloseBossBtn and event == cc.EventCode.ENDED then  
        --退出公会BOSS
        self.m_UnionBg:setVisible(true)
	...
	elseif sender == self.m_UnionCloseButton and event == cc.EventCode.ENDED then
        ...
        self.m_InputUnionName:getDetachWithIME()
        ...
        if string.len(UnionNamestring) == 0 or string.len(UnionNamestring) > 6 then
            ...
            Http:req("CreatGuildRequest",{guildName=UnionNamestring},function(res)
                ...
                elseif res.data["result"] == 3 then
                    ...
                    self.m_Sure2Union:setVisible(false)
                    self.m_CoverPanel:setVisible(false) 
                elseif res.data["result"] == 26 then
                    PopDialog:getInstance():popText("创建失败，当前您已加入其他公会",cc.c3b(0,255,0))
                    self.m_Sure2Union:setVisible(false)
                    self.m_CoverPanel:setVisible(false)      
                end
            end)
	...
	elseif sender == self.m_CorrectNoticeBtn and event == cc.EventCode.ENDED then
        ...
        if string.len(UnionNoticestring) == 0 then
            PopDialog:getInstance():popText("输入内容不合法！请重新输入",cc.c3b(0,255,0))
        else
                ...
                m_touxiangkuang:loadTexture("soul/"..rse.data.guildRolerList[i].avatarId..".png")
                    ...
                    if res.data.guildInfo.guildPos == 1 then
                        ...
                        elseif rse.data.guildRolerList[i].guildPos == 3 then
                            m_UnionMemberPosition:setString("成员")
                            m_OperateMemberButton:setVisible(true)
    ...
    Http:req("GuildListRequest",nil,function(res)
        ...
        if UnionNumber > 4 then
            if UnionNumber%2 == 0 then
                ScrovH = (190)*(UnionNumber/2)
            else
                ScrovH = (190)*(math.floor(UnionNumber/2)+1)
            end
        end
        ...
        for i=1,UnionNumber do
            ...
            local m_refuseAnyone = ccui.Helper:seekWidgetByName(UnionItem, "Button_10")
            ...
            if res.data.guildList[i].memberNum ~= res.data.guildList[i].maxMember then--判断是否可以加入
                ...
                m_refuseAnyone:setVisible(false)
            else
            ...
            if res.data.guildList[i].applyType == 2 then
                m_ApplyUnionBtn:setVisible(false)
                m_UnionPeopleFull:setVisible(false)
                m_AlreadyApply:setVisible(false)
                m_refuseAnyone:setVisible(true)
            end
                ...
                if sender == m_ApplyUnionBtn and event == cc.EventCode.ENDED then
                    Http:req("ApplyGuildRequest",{guildId = res.data.guildList[i].guildId},function(res)
                        ...
                        elseif res.data.result == 7 then 
                            PopDialog:getInstance():popText("12小时内不能加入退出的的公会，1小时内不能加入其他公会",cc.c3b(0,255,0))
                            m_ApplyUnionBtn:setVisible(true)
                            m_UnionPeopleFull:setVisible(false)
                            m_AlreadyApply:setVisible(false)
                            m_refuseAnyone:setVisible(false)
                        ...
                        elseif GuildTypeNum == 2 then 
                            ...
                            m_AlreadyApply:setVisible(false)
                            m_refuseAnyone:setVisible(false)
                        elseif GuildTypeNum == 26 then 
                            PopDialog:getInstance():popText("申请失败，当前您已经成功加入其他公会",cc.c3b(0,255,0))
                            m_ApplyUnionBtn:setVisible(true)
                            m_UnionPeopleFull:setVisible(false)
                            m_AlreadyApply:setVisible(false) 
                            m_refuseAnyone:setVisible(false)    
...
function UnionUi:UnionFood(eatNumber)
    ...
    Http:req("EatGuildDinnerRequest",{eatId = EatNum},function(res)
        ...
        elseif res.data.result == 0 then 
            if res.data.eatId == 1 then
                --lable11
                PopDialog:getInstance():popText("品尝了可乐，获得了"..dinnerConfig["1"]["energy"].."点体力，"..dinnerConfig["1"]["guild_devote"].."点个人贡献",cc.c3b(0,255,0))
            elseif res.data.eatId == 2 then
            --lable113
                PopDialog:getInstance():popText("品尝了汉堡，获得了"..dinnerConfig["2"]["energy"].."点体力，"..dinnerConfig["2"]["guild_devote"].. "点个人贡献",cc.c3b(0,255,0))
            elseif res.data.eatId == 3 then
            --lable15 
                PopDialog:getInstance():popText("品尝了烤鸡，获得了 "..dinnerConfig["3"]["energy"].." 点体力，"..dinnerConfig["3"]["guild_devote"].."点个人贡献",cc.c3b(0,255,0))
        ...
        elseif res.data.result == 3 then
            if EatNum == 1 then
                PopDialog:getInstance():popText("品尝失败！金币不足",cc.c3b(0,255,0))
            elseif EatNum == 2 then
                PopDialog:getInstance():popText("品尝失败！钻石不足",cc.c3b(0,255,0))
            elseif EatNum == 3 then 
                PopDialog:getInstance():popText("品尝失败！钻石不足",cc.c3b(0,255,0))
            end       
        end
    end)
...
function UnionUi:UnionShopFace()
    Http:req("GetGuildShopRequest",nil,function(res)
        ...
        for i_2=1,ItemNumber do
            ...
            local suipian = ccui.Helper:seekWidgetByName(UnionShopItem, "suipian")
            local pinzhikuang = ccui.Helper:seekWidgetByName(UnionShopItem, "pinzhikuang")
            --判断是否已购买
            ...
            pinzhikuang:loadTexture("normal_aochao_0"..shopConfig[tostring(ItemID)]["quality"]..".png")
            pinzhikuang:setScale(1.1,1.1)
            
            suipian:setVisible(false)
            
            if shopConfig[tostring(ItemID)]["type"] == 16 then
                         ...
                         elseif sender == self.m_confirm4 and event == cc.EventCode.ENDED then 
                            Http:req("BuyGuildShopRequest",{id = res.data.item[i_2].id },function(re)
                                if re.data.result == 0 then
                                    ...
                                    m_ItemCover:setVisible(true)
                                elseif re.data.result == 16 then
                                    PopDialog:getInstance():popText("兑换失败，您的贡献币不足",cc.c3b(0,255,0))    
                                end
                            end) 
                            self.m_Sure2Change:setVisible(false)
                         end
...
function UnionUi:UnionWareHouse()
    ...
    Http:req("GetGuildBagRequest",nil,function(res)
            ...
            local copyitem = ccs.GUIReader:getInstance():widgetFromBinaryFile("UnionCangKuItem.csb")
            local listviewsize = self.m_CangkuListView:getContentSize()
            listviewsize.height = copyitem:getContentSize().height*(#myList)+15*(#myList-1)
            self.m_CangkuListView:setInnerContainerSize(listviewsize)
            ...
            for i=1,HouseItemNumber do
                ...
                local cangkusuipian = ccui.Helper:seekWidgetByName(HouseItem, "cangkusuipian")
                
                local function taskItemTouchFunc()
                    SetSlider(self.m_CangkuListView,self.m_percentSlider)        
                end
                HouseItem:addTouchEventListener(taskItemTouchFunc)
                
                m_touxiangK:loadTexture(shopConfig[tostring(res.data.item[i].itemId)]["icon"]..".png")
                m_touxiangK:setScale(0.7,0.7)
                m_touxiangKK:loadTexture("normal_aochao_0"..shopConfig[tostring(res.data.item[i].itemId)]["quality"]..".png")
                ...
                if shopConfig[tostring(res.data.item[i].itemId)]["type"] == 16 then
                    cangkusuipian:setVisible(true)
                end
                
                --[[tostring(res.data.item[i].itemId)]]  --服务端更新后替换
                m_ExchangePrice:setString(UnionBagItemConfig[tostring(res.data.item[i].itemId)]["score"])
                ...
                elseif res.data.item[i].applyed == 1 then --已申请
                    ...
                    self.IsApply = self.IsApply + 1
                    
                    --二次确认框内容
                    local AlreadyLable = shopConfig[tostring(res.data.item[i].itemId)]["name"]
                    self.m_AlreadyApplyItem:setVisible(false)
                    ...
                    richtext:setPosition(cc.p(-315,80))
                    
                    richtext2:ignoreContentAdaptWithSize(false)
                    richtext2:setContentSize(cc.size(700,30))
                    richtext2:setAnchorPoint(cc.p(0,0))
                    richtext2:setPosition(cc.p(-315,43))
                   
                   self.fontpanel:removeAllChildren()
                   
                    local richitem2 = ccui.RichElementText:create(2,cc.c3b(247,247,15),255,AlreadyLable,"黑体",22)
                    local richitem1 = ccui.RichElementText:create(1,cc.c3b(255,255,255),255,"你已在","黑体",22)
                    local richitem3 = ccui.RichElementText:create(3,cc.c3b(255,255,255),255,"的装备排队申请","黑体",22)
                    
                    local richitemnew2 = ccui.RichElementText:create(2,cc.c3b(247,247,15),255,AlreadyLable,"黑体",22)
                    local richitemnew1 = ccui.RichElementText:create(1,cc.c3b(255,255,255),255,"是否离开","黑体",22)
                    local richitemnew3 = ccui.RichElementText:create(3,cc.c3b(255,255,255),255,"的申请","黑体",22)

                    richtext:pushBackElement(richitem1)
                    richtext:pushBackElement(richitem2)
                    richtext:pushBackElement(richitem3)
                    
                    richtext2:pushBackElement(richitemnew1)
                    richtext2:pushBackElement(richitemnew2)
                    richtext2:pushBackElement(richitemnew3)
                    
                    self.fontpanel:addChild(richtext)
                    self.fontpanel:addChild(richtext2)
                    --self.m_AlreadyApplyItem:setColor(cc.c3b(247,247,15))
                    self.m_AlreadyApplyItem_2:setVisible(false)    
                end
                ...
                local function WareHouse2CallBack(sender,event)--排队操作按钮
                    ...
                    if sender == m_DetailBtn and event == cc.EventCode.ENDED then
                        ...
                        self.ItemtouxiangPinzhi:loadTexture("normal_aochao_0"..shopConfig[tostring(res.data.item[i].itemId)]["quality"]..".png")
                    ...
                    elseif sender == m_NoBtn and event == cc.EventCode.ENDED then
                        ...
                        self:ApplyGuildBag(res.data.item[i].itemId,m_YesBtn,m_NoBtn)
                            ...
                            local notLable = tostring(shopConfig[tostring(res.data.item[i].itemId)]["name"])
                            self.m_donotApplyItem:setVisible(false)
                            
                            self.richtextnot = ccui.RichText:create()
                            
                            self.richtextnot:ignoreContentAdaptWithSize(false)
                            self.richtextnot:setContentSize(cc.size(700,30))
                            self.richtextnot:setAnchorPoint(cc.p(0,0))
                            self.richtextnot:setPosition(-315,5) 
                            local richitem2 = ccui.RichElementText:create(2,cc.c3b(247,247,15),255,notLable,"黑体",22)
                            local richitem1 = ccui.RichElementText:create(1,cc.c3b(255,255,255),255,"加入到","黑体",22)
                            local richitem3 = ccui.RichElementText:create(3,cc.c3b(255,255,255),255,"的申请？","黑体",22)
        
                            self.richtextnot:pushBackElement(richitem1)
                            self.richtextnot:pushBackElement(richitem2)
                            self.richtextnot:pushBackElement(richitem3)
                            self.fontpanel:addChild(self.richtextnot)
                            
                            self.m_sure2Cangku:setVisible(true)
                            self.cangkupanel:setVisible(true) 
            ...
            self.m_CangkuListView:jumpToTop() 
    end)
...
function UnionUi:DetailGuildBag(itemIdNum)
    Http:req("DetailGuildBagItemRequest",{itemId = itemIdNum},function(res)
        ...
        local copyitem = ccs.GUIReader:getInstance():widgetFromBinaryFile("ApplyCangkuMember.csb")
        local listviewsize = self.m_BagDetailListView:getContentSize()
        listviewsize.height = copyitem:getContentSize().height*(#myList)+15*(#myList-1)
        self.m_BagDetailListView:setInnerContainerSize(listviewsize) 
        ...
        self.m_BagDetailListView:jumpToTop()
    end)
end
            ...
            local function AttackStage2CallBack(sender,event)
                if sender == StageItem and event == cc.EventCode.ENDED then
                                ...
                                local function AttackStage2CallBack_2(sender,event)
                                    if sender == self.m_confirm9 and event == cc.EventCode.ENDED then 
                                        Http:req("OpenGuildBigStageRequest",{id = res.data.info[i].id},function(re)
                                            if re.data.result == 0 then
                                                PopDialog:getInstance():popText("开启成功！",cc.c3b(0,255,0))
                                                --ProgressBar:setVisible(true)
                                                --刷新大关卡
                                                self:UnionBossInterface()
                  ...
                  elseif res.data.info[i].state == 2 then 
                     ...
                     self.IndexBossID = res.data.info[i].id
                     --self.m_UnionBg:setVisible(false)
...
function UnionUi:SmallBossInterface(BigBossId)
    ...
    local restAttackNum = res.data.maxAtkNum - res.data.atkNum
    
    if restAttackNum < 0 then
        restAttackNum = 0
    end
    self.AttackNumber:setString(restAttackNum.."/"..res.data.maxAtkNum)
    
    if  self.BtnScrollView:getChildrenCount() ~= 0 then 
        self.BtnScrollView:removeAllChildren()
    end
    local ScrovSize = self.BtnScrollView:getInnerContainerSize()
    
    for i = 1,7 do
        local smallStage = ccs.GUIReader:getInstance():widgetFromBinaryFile("BossItem.csb")
        local SmallStageItem = ccui.Helper:seekWidgetByName(smallStage, "BtnItem")
        local AtlasLabel = ccui.Helper:seekWidgetByName(smallStage, "AtlasLabel")
        SmallStageItem:setTag(i)
        local m_boss_progress = ccui.Helper:seekWidgetByName(smallStage, "m_boss_progress")
        local m_boss_name = ccui.Helper:seekWidgetByName(smallStage, "m_boss_name")
        local m_boss_pro = ccui.Helper:seekWidgetByName(smallStage, "m_boss_pro")
        
        local SmallStageIdNum = (SmallStageId-1)*7+i
        
        AtlasLabel:setString(i-1)
        m_boss_name:setString(SmallStageConfig[tostring(SmallStageIdNum)]["stage_name"])
            ...
            if SmallStageIdNum == res.data.stageId then
                ...
                --SmallStageItem:loadTextures("yuankuang.png","yuankuang01.png","yuankuang.png")
                SmallStageItem:setEnabled(true)
                m_boss_pro:setVisible(true)
            elseif SmallStageIdNum < res.data.stageId then
                m_boss_progress:setPercent(100)
                --Helper.graySprite(SmallStageItem:getVirtualRenderer())
                SmallStageItem:setEnabled(false)
            ...
            if i > 1 and i <= 4 then
                    smallStage:setPosition(175*(i-2)+250,182)
            elseif i == 1 then 
                    smallStage:setPosition(75,182)   
            else
                    smallStage:setPosition(175*(i-4),4)    
            end
            
            self.BtnScrollView:addChild(smallStage)
            
            local function StartFight2CallBack(sender,event)
                if sender == SmallStageItem and event == cc.EventCode.ENDED then
                
                        if restAttackNum == 0 then
                        PopDialog:getInstance():popText("今日挑战次数已满！",cc.c3b(0,255,0))
                    else
                        ...
                        --显示掉落物品icon
                        local monsterDec = SmallStageConfig[tostring(res.data.stageId)]["fourth_monster"]
                        self:ItemPicture(SmallStageConfig[tostring(res.data.stageId)]["item"],monsterDec)
...
--显示掉落物品icon及关卡怪物函数
function UnionUi:ItemPicture(lable,lable_1)--lable 掉落物品1  --lable_1 关卡怪物2
    --物品
    local input = tostring(lable)
    local res = string.split(input, ";")
    
    --怪物
    local input_1 = tostring(lable_1)
    local res_1 = string.split(input_1, "|")
    
    local LableLen = table.getn(res) --1
    local LableLen_1 = table.getn(res_1) --2
    
    for j = 1,4 do --物品初始化
        local m_goodsNew = ccui.Helper:seekWidgetByName(self.m_prepare2Battle, "m_goods"..j)
        m_goodsNew:setVisible(false)
    end
    
    for j_1 = 1,4 do --怪物初始化
        local m_monsterPicNew = ccui.Helper:seekWidgetByName(self.m_prepare2Battle, "m_monsterPic"..j_1)
        m_monsterPicNew:setVisible(false)
    end
    ...
    for i = 1,LableLen do
        ...
        m_goods:setVisible(true)
        local goodsKuang = ccui.Helper:seekWidgetByName(m_goods, "goodsKuang")
        m_goods:loadTexture(shopConfig[tostring(res_2[1])]["icon"]..".png")
        goodsKuang:loadTexture("normal_aochao_0"..shopConfig[tostring(res_2[1])]["quality"]..".png")
    end
    
    local pictable = {}
    
    for i_1 = 1,LableLen_1 do
        local arr_1 = res_1[i_1+1]
        local res_21 = string.split(arr_1, ",")
        table.insert(pictable,res_21[1])
    end
    
    local picNum = table.getn(pictable)
    
    for i = 1,picNum-1 do
        for j = i +1,picNum do
            if pictable[i] == pictable[j] then
                table.remove(pictable,j)
                picNum = picNum - 1
            end
        end
    end 
    
    local picN = table.getn(pictable)
    if picN > 4 then
        picN = 4
    end
    
    for i = 1,picN do
        local m_monsterPic = ccui.Helper:seekWidgetByName(self.m_prepare2Battle, "m_monsterPic"..i)
        m_monsterPic:setVisible(true)
        local monsterKuang = ccui.Helper:seekWidgetByName(m_monsterPic, "monsterKuang")
    
        --暂无头像 表配好更换
        m_monsterPic:loadTexture(soulConfig[tostring(pictable[i])]["icon"]..".png")
        monsterKuang:loadTexture("normal_aochao_0"..soulConfig[tostring(pictable[i])]["quality"]..".png")
    end
end
