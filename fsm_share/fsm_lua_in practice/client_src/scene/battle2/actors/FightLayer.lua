...
function FightLayer:initM()
    ...
    self.cmdHandlers = {
        ...
        ["walk_o"] = function (cmd) self:walk_o(cmd) end,
        ...
        ["leidian_s"] = function (cmd) self:leidian_s(cmd) end, --skillid 16
        ["leidian_o"] = function (cmd) self:leidian_o(cmd) end,
    ...
    --遮罩
    self.colorLayer = cc.LayerColor:create(cc.c4b(0,0,0,0))
...
function FightLayer:walk_o(cmd)
    self:resetZOrder()
end
...
function FightLayer:move(cmd)
    ...
    if self.resetZOrderCount % 30 == 0 then
        self:resetZOrder()
    end
    ]]--check zOrder --
end
...
function FightLayer:leidian_s(cmd)
    local npc = self:getNpc(cmd.key)
    if npc then
        ...
        armature:setScaleY((WIN_SIZE.height-npc:getPositionY())/(WIN_SIZE.height/2)*0.72)
        armature:setScaleX((WIN_SIZE.height-npc:getPositionY())/(WIN_SIZE.height/2)*0.72)
        armature:setName("leiyun")
      -- armature:setLocalZOrder(npc:getLocalZOrder()+1)
        --armature:setPosition(cc.p(npc:getPositionX(),npc:getPositionY()))
        npc:addChild(armature)
        armature:getAnimation():play("Animation1")
...
function FightLayer:leidian_o(cmd)
    --[[
    local npc = self:getNpc(cmd.key)
    if npc then
        local leiyun = npc:getChildByName("leiyun")
        if  leiyun then
            leiyun:removeFromParent()
        end  
    end
    ]]
end
