...
function HeroSkillBtn:enable()
    ...
    if self.percentage and self.percentage >= 100 and self.isDie==false and self.isSilence==nil and self.isFireNo==nil then
        self.btn:setTouchEnabled(true)
        self.ring:setVisible(true)
    end
end
...
function UI2:initM(param)
    ...
    if param.battleType == 2 then 
        fireModeBtn:loadTextureNormal("image/battle/battleuilayer2/battle_shoudong01.png", 0)
        fireModeBtn:loadTexturePressed("image/battle/battleuilayer2/battle_shoudong02.png", 0)
        fireModeBtn:setTouchEnabled(false)
    elseif param.battleType == 1 then
        if self.fireMode == 0 then
            fireModeBtn:loadTextureNormal("image/battle/battleuilayer2/battle_zidong01.png", 0)
            fireModeBtn:loadTexturePressed("image/battle/battleuilayer2/battle_zidong02.png", 0)
        else
            fireModeBtn:loadTextureNormal("image/battle/battleuilayer2/battle_shoudong01.png", 0)
            fireModeBtn:loadTexturePressed("image/battle/battleuilayer2/battle_shoudong02.png", 0)
    ...
    if self.battleType == 1 then
        btn:addClickListener(function(param) 
            ...
            if directorAgent:canFireSkill(btn.id) then
                ...
                print("[UI2]BigSkill can not be Fired: "..param.id) 
            end
        end)
    end
end
...
function UI2:enableAllBigSkill()
    for k, v in pairs(self.heroSkillBtns) do
        if v and v.isSilence==nil and v.isFireNo==nil then
            v:enable()
        end
    end
end
...
function UI2:enableBigSkill(soulId)
    local btn = self.heroSkillBtns[soulId]
    if btn and btn.isSilence==nil and btn.isFireNo==nil then
        btn:enable()
    end
end
