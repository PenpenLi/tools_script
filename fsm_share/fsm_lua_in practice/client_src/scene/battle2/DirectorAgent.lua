...
function DirectorAgent:suspendForBigSkill(fromDirector)
    ...
    self.suspendSeq = self.suspendSeq or 0
    self.suspendSeq = self.suspendSeq + 1
    if self.fightMode ==1 and fromDirector.direction~=1 then return end
    local pauseDt = math.min(fromDirector:getBigSkillOverTime(), 45)
    local pauseDt = math.max(pauseDt, 10)
    for i=1, #self.directors do
        if self.directors[i] ~= fromDirector then
            self.directors[i]:rest({dt=pauseDt, seq=self.suspendSeq})
        end
    end
end

function DirectorAgent:suspendForJX(fromDirector, dt)
    --pve battle only left hero fire skill lead to black screen
    self.suspendSeq = self.suspendSeq or 0
    self.suspendSeq = self.suspendSeq + 1
    if self.fightMode ==1 and fromDirector.direction~=1 then return end
...
function DirectorAgent:canFireSkill(soulId)
    ...
    for i=1, #self.directors do
        if self.directors[i].npcBook.direction == 1 and self.directors[i].npcBook.soulId == soulId then
            if self.directors[i]:canBigSkill()==true then
                return true
            else
                return false
            end
    ...
    print("[DirectorAgent]canFireSkill: no director for soulid "..soulId)
    return false
end
...
function DirectorAgent:fireJXSkill(data)
    ...
    actorAgent:actCMD({t="jx_dark_s", type=skillid, p=0, direction=data.direction})
    self:setTimeout(20, function()
        actorAgent:actCMD({t="jx_dark_o", type=skillid, p=0, direction=data.direction})
    end)
    
    --Dian Ci Zhen Dang, Huo Pao Hong Zha
    if skillid == 1 or skillid == 2 then
        data.hp = jx["zhili"]*jx["hp_factor"]
        data.seq = self.jxSeq
        data.dt = jx["dt"]/1000
        data.hp_factor = jx["hp_factor"]
        data.slice_num = jx["slice_num"]
        data.slice_dt = jx["slice_dt"]
        data.slice_hp_factor = jx["slice_hp_factor"]
        for i=1, #self.directors do
            if skillid == 1 and self.directors[i].direction == data.direction then
                self.directors[i]:onCiBao(data)
            elseif skillid == 2 and self.directors[i].direction == -1*data.direction then
                self.directors[i]:onHongZha(data)
            end
            self:suspendForJX(self.directors[i], 45)
        end
    --cibao_s  hongzha_s
    ...
    elseif skillid == 4 then
        ...
        self:addDirector(towerNpcBook)
        local director = self:getDirector(towerNpcBook.key)
        director.recruit_num = jx["recruit_num"]
        director:stop()
        self:setTimeout(60, function()
            director:restart()
        end)
    ...
    elseif skillid == 5 then
        for i=1, #self.directors do
            if self.directors[i].direction == -1*data.direction then
                ...
                data.seq = self.jxSeq
                data.dt = jx["dt"]/1000
                data.add_miss_factor = jx["add_miss_factor"]
                self.directors[i]:onYanWu(data)
                self:suspendForJX(self.directors[i], 15)
            end
        end
    --[[
    --Zhao Huan Fei Ting
    elseif skillid == 6 then
        ...
        enemys[j]:onZhaoHuanFeiTing(data)
    ]]
    ...
    elseif skillid == 7 then
        ...
        for i=1, #self.directors do
            if self.directors[i].direction == 1*data.direction then
                self.directors[i]:onGPS(data)
                self:suspendForJX(self.directors[i], 20)
            end
        end
    ...
    elseif skillid == 8 then
        ...
        data.seq = self.jxSeq
        data.dt = jx["dt"]/1000
        data.add_atk_factor = jx["add_atk_factor"]
        data.add_def_factor = jx["add_def_factor"]
        
        for i=1, #self.directors do
            if self.directors[i].direction == 1 then
                self.directors[i]:onTuTeng(data)
                self:suspendForJX(self.directors[i], 20)
    ...
    elseif skillid==9 then
        local hp = jx["zhili"]*jx["hp_factor"]
        for i=1, #self.directors do
            if self.directors[i].direction == 1*data.direction then
    ...
    elseif skillid==12 then
        ...
        for i=1, #self.directors do
            if self.directors[i].direction == 1*data.direction then
                self.directors[i]:onShengGuang(data)
                self:suspendForJX(self.directors[i], 15)
            end
        end
    ...
    elseif skillid == 15 then
        ...
        local targetIndex = 0
        local distance2 = 99999
        local targetIndex2 = 1
        for i=1, #enemys do
            local tmpDistance = math.abs(enemys[i].position.x-480)
            if enemys[i].npcBook.soulId==9000 or enemys[i].npcBook.soulId==9001 or 
                enemys[i].npcBook.soulId==9002 or enemys[i].npcBook.soulId==9005 or
                enemys[i].npcBook.soulId==9006 or enemys[i].npcBook.soulId==9007 then
                if tmpDistance < distance then
                    distance = tmpDistance
                    targetIndex = i
                end
            else
                if tmpDistance < distance2 then
                    distance2 = tmpDistance
                    targetIndex2 = i
                end
            end
        end
        ...
        if enemys[targetIndex] then
            data.hp = jx["zhili"]*jx["hp_factor"]
            data.seq = self.jxSeq
            data.dt = jx["dt"]
            enemys[targetIndex]:onLeiDian(data)
            self:suspendForJX(enemys[targetIndex], 20)
        end
    ...
    elseif skillid==14 then
        ...
        data.dt = jx["dt"]/1000
        for i=1, #enemys do
            enemys[i]:onBaoFengXue(data)
        end
    ]]
    
    elseif skillid==13 or skillid==14 then
        data.dt = 3
        data.hp = jx["zhili"]*jx["hp_factor"]
        data.seq = self.jxSeq
...
function DirectorAgent:getTarget(director)
    ...
    if director.npcBook.name == "ZhaoHuanFaZhen_juexingSkill" then
        return director
    end
...
function DirectorAgent:getStandInfo(director)
    ...
    if standCelly > 6 then
        standCelly = 6
    end
    if standCelly < 1 then
        standCelly = 1
    end
...
function DirectorAgent:onAttack(fromKey, targetKey, hp)
    ...
    if target then
        if fromKey == targetKey and target.npcBook.name == "ZhaoHuanFaZhen_juexingSkill" then
            if target.recruit_num==nil then return end
            target.recruit_num = target.recruit_num - 1
            if target.recruit_num <= 0 then return end
...
function DirectorAgent:getSmallSkillTargets(key)
    ...
    if director.npcBook.name == "Tyrande" or director.npcBook.name == "Jaina" or
        ...
        if director.npcBook.name == "Tyrande" then
            ...
            for i=1,#teammates do
                if  teammates[i].hp/teammates[i].npcBook.maxHp <= hp_per then
                    hp_per = teammates[i].hp/teammates[i].npcBook.maxHp
                    j = i
                end
            end
    ...
    for i=#smallSkillTargets,1,-1 do
        if smallSkillTargets[i].position.x > 960 or smallSkillTargets[i].position.x < 0 then
            table.remove(smallSkillTargets, i)
        else
            local distance = math.abs(smallSkillTargets[i].position.x - director.position.x)
...
function DirectorAgent:getBigSkillTargets(key)
    ...
    for i=#bigSkillTargets,1,-1 do
        if bigSkillTargets[i].position.x > 960 or bigSkillTargets[i].position.x < 0 then
            table.remove(bigSkillTargets, i)
        else
            local distance = math.abs(bigSkillTargets[i].position.x - director.position.x)
            if distance > director.npcBook.big_skill_range_x*cell_width or bigSkillTargets[i].npcBook.name=="mutong" then
                table.remove(bigSkillTargets, i)
            end
        end
    end
...
function DirectorAgent:gameOver()
    ...
    for i=1, #self.directors do     
        ...
        if director.direction == 1 then
            battleSoulLeft.soulId = director.npcBook.soulId
