...
local rechargeConfig = require("config.define_vip_recharge")
...
function RechargeUI:initM()
    ...
    self.m_left:addTouchEventListener(touchEventHandler)

	--m_right: ImageView
	self.m_right = ccui.Helper:seekWidgetByName(self.rootNode, "m_right")
    self.m_right:addTouchEventListener(touchEventHandler)
...
function RechargeUI:handleTouchEvent(sender, event)
    ...
    elseif sender == self.m_left and event == cc.EventCode.ENDED then
        cclog("[RechargeUI]--m_left touched--")
        local pageid = self.m_PageView:getCurPageIndex()
        self.m_PageView:scrollToPage(pageid-1)
        
    elseif sender == self.m_right and event == cc.EventCode.ENDED then
        cclog("[RechargeUI]--m_right touched--")
        local pageid = self.m_PageView:getCurPageIndex()
        self.m_PageView:scrollToPage(pageid+1)
...
function RechargeUI:initUI()
    Http:req("VipShopList",nil,function(res)
        ...
        local nextlevel = tonumber(res.data["vipLevel"])
        ...
        for i=1,RCsize do
            local rechargeID = tostring(RClist[i]["shopId"])
            local rechargeItem = rechargeConfig[rechargeID]
            local shopId = tostring(rechargeItem["shop_id"])
            ...
            if tonumber(rechargeItem["discount"]) == 0 then
                m_price:setString(rechargeItem["price"])
            else
                m_price:setString(rechargeItem["discount"])
            end
        end
    end)
end
