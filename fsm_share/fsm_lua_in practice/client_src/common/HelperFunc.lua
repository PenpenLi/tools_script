...
function Helper.calJXSkills(memberList)
    ...
    for _, v in pairs(memberList) do
        local soul = define_soul[tostring(v.soulId)]
        if soul then
            local blast_book_id = soul["blast_book_id"]
            if jxSkills[blast_book_id] == nil then
                jxSkills[blast_book_id] = {num=1, zhili=v.zhili}
            else
                jxSkills[blast_book_id] = {num=jxSkills[blast_book_id].num+1, zhili=jxSkills[blast_book_id].zhili+v.zhili}
            end
        end
    end
