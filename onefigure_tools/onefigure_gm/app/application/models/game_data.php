...
class Game_data extends CI_Model {
    ...
    public function selectAchievementInfo($data = array()) {
        log_message('error', 'selectAchievementInfo');
        ...
        $contents = $this->curl->execute();
        write_file('/tmp/docs_temp_file'.$version.'achievement', $contents);
        log_message('error', $docs['baseurl'].$version.'/'.$docs['filename']['achievement']);
        $docsRows = $this->docs_reader->parse_file('/tmp/docs_temp_file'.$version.'achievement');
