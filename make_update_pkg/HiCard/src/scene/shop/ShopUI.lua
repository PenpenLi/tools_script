    ...
    Http:req("GetShopList",nil,function(res)
        ...
        if shopsize>4 then
            if shopsize%2 == 0 then
                ScrovH = (161+5)*(shopsize/2)
            else
                ScrovH = (161+5)*(math.floor(shopsize/2)+1)
            end
        ...
        for i=1,shopsize do
            local shopId=tostring(shoplist[i]["shopId"])
            
            local shopitem = ccs.GUIReader:getInstance():widgetFromBinaryFile("ShopUI_One.csb")
            shopitem:setPosition(2+(i-1)%2*293,ScrovSize.height-165*(math.floor((i-1)/2)+1))
            ...
            local function ShopMenuCallBack(sender,event)
                if sender == m_buybtn and event == cc.EventCode.ENDED then
                    ...
                    if  shopConfig[shopId] ~= nil then
                     ...
                     if tonumber(dataitem["money_type"])== 0 then
                    ...
                    if tonumber(dataitem["discount"])== 0 then
                            self.m_totalPriLabel:setString(dataitem["price"])
                            self.oneprice = tonumber(dataitem["price"])
                    else
                            self.m_totalPriLabel:setString(dataitem["discount"])
                            self.oneprice = tonumber(dataitem["discount"])
                    end
            ...
            if  shopConfig[shopId] ~= nil then
                ...
                if tonumber(dataitem["discount"])== 0 then
                    ...
                    m_price:setString(dataitem["discount"])
                end
